...
namespace LCBaseMod{
    ...
    public  class LCBM_Tools_CDL
    {
        ...
        public static CreatureTypeInfo LoadCreatureTypeInfo(XmlDocument doc, ref List<CreatureSpecialSkillTipTable> creatureSpecialSkillTipList, ref Dictionary<long, int> specialTipSizeLib, out ChildCreatureData childData)
        {
            ...
            {
                id = long.Parse(xmlNode.Attributes.GetNamedItem("id").InnerText)
            };
            ...
            if (xmlNode7 != null)
            {
                ...
                try
                {
                    while (enumerator.MoveNext())
                    {
                        ...
                        {
                            key = xmlNode8.Attributes.GetNamedItem("key").InnerText
                        };
            ...
            )
        {
            ...
            XmlNodeList CreatureNodes = VanillaGameCreatureList.SelectNodes("/creature_list/creature");//提取节点列表
    ...
    public class LCBM_Tools_Spine
    {
        ...
        public static AtlasAsset CreateAtlaAssetRuntimeInstance(string atlasTextData, Texture2D[] textures, Material materialPropertySource, bool initialize)
        {
            ...
            if (initialize)
            {
                ...
                Atlas _atlas = new Atlas(new StringReader(text), string.Empty, new MaterialsTextureLoader(atlasAsset));
                _atlas.FlipV();
